...
public interface QnaReplyRepository extends JpaRepository<QnaReplyEntity, Long> {
    ...
    @Query(value = "SELECT * FROM qna_reply_table WHERE id IN (SELECT reply_id FROM (SELECT reply_id, COUNT(*) likeCount FROM qna_reply_like_table GROUP BY reply_id ORDER BY likeCount DESC) WHERE ROWNUM <= 5)", nativeQuery = true)
